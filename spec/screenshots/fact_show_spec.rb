...
describe "factlink", type: :feature do
  ...
  before do
    ...
    as(@user) do |pavlov|
      pavlov.interactor(:'facts/set_interesting', fact_id: @factlink.fact_id.to_s)
    end

    as(@user) do |p|
      c = p.interactor(:'comments/create', fact_id: @factlink.fact_id.to_s, type: 'believes', content: comment_text)
    ...
    as(@user) do |p|
      c = p.interactor(:'comments/create', fact_id: @factlink.fact_id.to_s, type: 'believes', content: "Henkie!")
  ...
  it "the layout of the new discussion page is correct with believers on top,
    ...
    find('.spec-topbar-menu-arrow').click #open menubar for screenshot test.
